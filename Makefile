...
lint:  ## run static analysis with pylint
	python3 -m black --check main.py tests.py setup.py
	python3 -m pylint main.py tests.py setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python3 -m black main.py tests.py setup.py
...
coverage:  ## clean and run unit tests with coverage
	python3 -m coverage run -m unittest tests.py && coverage report
